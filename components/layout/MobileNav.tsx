...
import { LayoutDashboard, Users, ShoppingCart, Shield, Globe, Trophy } from 'lucide-react'
...
const nav = [
  ...
  { href: '/matchday/1', icon: Globe,            label: 'Mondiale' },
]
    ...
    <>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-[#161b22] border-t border-gray-200 dark:border-gray-800 flex">
        {nav.map(({ href, icon: Icon, label }) => {
          const baseHref = href.startsWith('/matchday') ? '/matchday' : href
          const active = pathname === href || (href !== '/dashboard' && pathname.startsWith(baseHref))
          return (
